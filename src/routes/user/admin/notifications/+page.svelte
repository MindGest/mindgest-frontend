...
<div class="flex flex-col justify-center h-auto w-5/6 m-10 p-5 width = 20%">
  ...
  {#each notif as n}
    {#if n.lido === false}
      <wrapper
        class="flex flex-col"
        style="margin: 20px; float:left; width:30%; position:relative; top:10%"
      >
        <p>{n.titulo}</p>
      </wrapper>
      <wrapper class="flex flex-col" style="float:right; width:20%; position:relative; left:30%">
        <Button on:click={changeReadability(n)} style="position:relative; left:60%; width:10%"
  ...
  {#each notif as n}
    {#if n.lido === true}
      <wrapper
        class="flex flex-col"
        style="margin: 10px; float:left; width:30%; position:relative; top:10%"
      >
        <p>{n.titulo}</p>
        <Button on:click={changeReadability(n)} style="position:relative; left:60%; width:10%"
          >Marcar como não lido</Button
        >
      </wrapper>
    {/if}
  {/each}
