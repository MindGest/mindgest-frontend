<script>
    ...
    async function filter(label){
        ...
        for (var i = 0; i < info.length; i++) { 
            if((infoNotRelevant[i]['pago']==true && payedChecked==true) || (infoNotRelevant[i]['pago']==false && notPayedChecked==true)){
    ...
    onMount(async () => {
        ...
        if (response.ok) {
            ...
            for (var i = 0; i < jsonInfo.length; i++) { 
                ...
                let stringDate = `${dateNumbers[2]} de ${api.getMonth(dateNumbers[1])} de ${dateNumbers[0]}`;
                infoHelper.push({'Utente': jsonInfo[i]["patientName"], 'Responsável(eis)':jsonInfo[i]["mainTherapist"], 'Código da Consulta':jsonInfo[i]["ref"], "Dia da Consulta": stringDate});
                infoNotRelevant.push({'pago': jsonInfo[i]["pago"],'custo': jsonInfo[i]["custo"] ,'estado': jsonInfo[i]["estado"], 'email':jsonInfo[i]["email"], 'nif':jsonInfo[i]["nif"], 'sns':jsonInfo[i]["sns"], 'morada':jsonInfo[i]["morada"]})
            }
            info = infoHelper;
            infoFiltered = info;
            infoNotRelevantFiltered = infoNotRelevant;
...
{:else}
    <div class="mx-20 my-10">
        <SearchBar hint="Pesquisa" filter={filtertText}/>
        <div>
            <Checkbox label="Por pagar" checked={true} filter={filter}/>
            <Checkbox label="Pago" checked={true} filter={filter}/>
        </div>
    </div>
