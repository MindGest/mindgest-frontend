...
<!-- dropdown buttons -->
<wrapper class="flex flex-col" style="margin: 100px; float:left; width:30%; position:relative; top:10%">
    <section class="menu-cont">
        <select class="menu bg-white text-gray-800 font-semibold py-2 px-4 border border-gray-400" name="filtro_room" id="filtro_room" bind:value={selectedRoom}>
    ...
    <section class="menu-cont">
        <select class="menu bg-white text-gray-800 font-semibold py-2 px-4 border border-gray-400" style="margin-top:10%" name="filtro_terp" id="filtro_terp" bind:value={selectedTerp}>
    ...
    <section class="menu-cont">
        <select class="menu bg-white text-gray-800 font-semibold py-2 px-4 border border-gray-400" style="margin-top:10%" name="filtro_hour" id="filtro_hour" bind:value={selectedHour}>
...
<!-- rooms list -->
<wrapper class="flex flex-col" style="float:right; width:30%; position:relative; top:20%; bottom:10%; right:10%">
            ...
            {#each list_apoint.sort(sortRooms("hour_beg")) as ap}
                ...
                {#if selectedTerp==="all" || !selectedTerp || selectedTerp.name===ap.terp.name}
                    ...
                    {#if r===ap.room && h>=ap.hour_beg && ap.hour_beg+ap.dur>h}
                        <button class="bg-orange-300 hover:bg-orange-100 text-gray-800 font-semibold py-2 px-3" on:click={isGuard} id={ap}>{h}h, {ap.terp.name}</button> 
                    {:else if r===ap.room && (selectedTerp==="all" || !selectedTerp)}
                        <button class="bg-green-300 hover:bg-green-100 text-gray-800 font-semibold py-2 px-3" on:click={isGuard} id="">{h}h</button>
                    {/if}
