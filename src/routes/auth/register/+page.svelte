...
<div class="flex flex-row items-center justify-center w-full h-auto border-b-4 space-x-30">
  ...
  <img src="/img/logo.png" alt="logo" class="p-3 object-cover h-40 w-40" />
</div>
...
<div class="flex flex-row items-start justify-center w-full m-10 p-5">
  <div class="flex flex-col items-center justify-center w-3/6 h-auto m-10 p-5 text-xl font-sans">
    ...
    {:else}
      <Avatar class="object-cover h-60 w-60" src="/img/logo.png" alt="" />
    {/if}
  ...
  <div class="flex flex-col justify-center h-auto w-5/6 m-10 p-5 items-stretch">
    <form class="flex flex-col w-full items-stretch" on:submit={onSubmit}>
      ...
      <TextBox
        ...
        bind:value={person.birthday}
        class="grid grid-cols-4 items-center font-bold"
      />
      <TextBox
        id="phone_number"
        label="Número de Telemóvel:"
        bind:value={person.cellphone}
        class="grid grid-cols-4 items-center font-bold"
      />
      <TextBox
        id="tax_number"
        label="NIF:"
        bind:value={person.tax_number}
        class="grid grid-cols-4 items-center font-bold"
      />
      {#if person.role.text == 'Terapeuta'}
        <TextBox
          ...
          bind:value={person.specialty}
          class="grid grid-cols-4 items-center font-bold"
        />
        <!--TODO: fazer um select com as especialidades ou guarda-se com "," (?)-->
        <TextBox
          id="opp"
          label="Cédula OPP:"
          bind:value={person.opp}
          class="grid grid-cols-4 items-center font-bold"
        />
      {/if}
      <TextBox
      ...
      <TextBox
        ...
        bind:value={password_validation}
        class="grid grid-cols-4 items-center font-bold"
      />
    </form>
    <div class="flex flex-row w-3/6 m-10 p-5 items-center justify-center space-x-56">
      <a class="text-orange-500 red w-full py-2 outline outline-1 rounded text-center" href="login"
        >Voltar ao Login</a
      >
