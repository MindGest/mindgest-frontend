...
<script>
  ...
  async function generate() {
    ...
    let body = {
      ...
      therapistId:
        data.therapist != NOFILTER ? parseInt(data.therapist.split(']')[0].slice(1)) : null
    };
    ...
    pdf.text('Estatísticas', 40, y);
    y = incrementY(pdf, y, 30);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(12);
    pdf.text('Data Inicial: ' + data.start, 40, y);
    y = incrementY(pdf, y, 30);
    pdf.text('Data Fim: ' + data.end, 40, y);
    y = incrementY(pdf, y, 30);

    records.forEach(proc => {
      pdf.setFontSize(16);
      pdf.text('Processo: ' + proc.id, 40, y);
      y = incrementY(pdf, y, 25);
      pdf.setFontSize(12);
      pdf.text('Especialidade: ' + proc.speciality, 40, y);
      y = incrementY(pdf, y, 25);
      pdf.text('Terapeuta Responsável: ' + proc.name, 40, y);
      y = incrementY(pdf, y, 25);
      pdf.text('Utente: ' + proc.patientName, 40, y);
      y = incrementY(pdf, y, 25);
      pdf.text('Total: ' + proc.total + ' €', 40, y);
      y = incrementY(pdf, y, 25);
      pdf.autoTable({
        startY: y,
...
{#if data != null}
  ...
  >
    <TextBox
      class="my-5 w-1/2 m-auto bg-ora"
    ...
    {#if data.role != THERAPIST}
      <Selector
        class="my-5 w-1/2 m-auto"
        label="Terapeuta"
        values={data.therapists}
        bind:value={data.therapist}
      />
    {/if}
