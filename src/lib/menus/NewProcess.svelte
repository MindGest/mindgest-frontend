  ...
  onMount(async () => {
    ...
    if (responsePatients.ok && responseTherapists.ok && responseSpecialities.ok) {
      ...
      patients.forEach(patient => {
        patients_names.push(formatPatient(patient));
      });
      ...
      specialities.forEach(speciality => {
        specialities_names.push(speciality.speciality);
      });
  ...
  <div class="grid grid-cols-2">
    <div class="m-auto flex flex-col w-2/3">
      ...
      {:else}
        <Button
          class="mt-10"
          text="Enviar pedido de criação de processo"
          on:click={requestRecord}
        />
      {/if}
    </div>
    <div>
      ...
      <div class="flex flex-line">
        <Selector
          class="my-2 w-2/3"
          values={options(data)}
          bind:value={data.patient}
          placeholder="Escolha um utente"
        />
      ...
      {#each data.selected_patients as patient}
        <div class="flex flex-line">
          ...
          <Button
            ...
            text="-"
            on:click={() => removePatientUI(getId(patient))}
          />
        </div>
      {/each}
    </div>
