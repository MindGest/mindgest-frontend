...
<script>
  ...
  onMount(async () => {
    ...
    if (
      responseTherapists.ok &&
      responseInterns.ok &&
      responseColaborators.ok &&
      responseProcessData.ok &&
      responseSpecialities.ok
    ) {
      ...
      specialities.forEach(spec => {
        specialities_names.push(spec.speciality);
      });

      let therapists_names = [];
      therapists.forEach(therapist => {
        therapists_names.push(therapist.name);
      });
  ...
  async function editRecord() {
    if (data.role != ADMIN && data.responsavel != data.new_responsavel) {
      ...
      let responseMigrate = await api.post(`/process/${data.processId}/migrate`, {
  ...
  <form class="grid grid-cols-2" on:submit|preventDefault={async () => await editRecord()}>
    <div>
      ...
      {#if data.role != ADMIN}
        <TextDisplay
          ...
          bind:value={data.responsavel}
        />
        <Selector
          class="my-5 w-1/2 m-auto"
          label="Pedido de alteração de responsável para"
          values={data.therapists_names}
          bind:value={data.new_responsavel}
        />
      {:else}
        <Selector
          class="my-5 w-1/2 m-auto"
    ...
    <div>
      <div class="flex flex-line m-4">
        ...
        {#if data.role != INTERN}
          <a
            class="w-3/6 text-orange-500 underline"
            href="/user/{data.role}/records/{data.processId}/edit/permissions"
            >Editar Permissões de Estagiários</a
          >
        {/if}
      </div>
      ...
      {#if data.role != INTERN}
        <div class="flex flex-line mb-10">
          <Selector
            class="m-auto w-2/3 p-4"
            values={colaboratorsOptions(data)}
            placeholder="Escolha um colaborador"
            bind:value={data.colaboratorAdd}
          />
      ...
      {#each data.colaborators.therapists as therapist}
        <div class="flex flex-line">
          ...
          {#if data.role != INTERN}
            <Button
              class="m-auto w-10 h-10"
              text="-"
              on:click={() => removeColaboratorUI('therapists', therapist.id)}
            />
          {/if}
        </div>
      {/each}
        ...
        <div class="flex flex-line">
          ...
          {#if data.role != INTERN}
            <Button
              class="m-auto w-10 h-10"
              text="-"
              on:click={() => removeColaboratorUI('interns', intern.id)}
            />
          {/if}
        </div>
      {/each}
